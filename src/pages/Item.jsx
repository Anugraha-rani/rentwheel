...
function Item({car}) {
   ...
   const currency = "₹";
    ...
    const id = car._id
  return (
    <div onClick={()=>navigate(`/details/${id}`)} style={{background:bgColor}} className='block rounded-lg ring-1 ring-slate-900/5 p-5 cursor-pointer h-95'>
       <h4 className='line-clamp-1 font-bold'>{car?.title}</h4>
       <div className='flex justify-between items-center'>
        <h5 className='my-1 text-gray-500'>{car?.bodyType}</h5>
        <div className='text-sm  text-solid'>
       ...
       <div>
        <div className='flex justify-evenly items-center py-2 text-gray-700'>
